...
def parse_gtf(gtf_file:str, gene_set:set):
    """
    parse_gtf function: parase genome gtf file.
    """
    print("parse_gtf")
    bed_records = []
    ...
    return bed_records
def parse_gtfForStringTie(gtf_file:str, gene_set:set):
    """
    parse_gtf function: parase genome gtf file.
    """
    print("parse_gtf")
...
def parase_gtfForTEcount(gtf_file:str, gene_set:set):
    ...
    return bed_records
def dropDuplicates(bed_records:list):
    """
    dropDuplicates function: drop the duplicates in the bed_records list.
    """
    print("dropDuplicates")
    seen = {}
    unique_records = []
    for record in bed_records:
        key = (record[0], record[1], record[2])  # (chrom, start, end)
        if key not in seen:
            seen[key] = True
...
def getBed(infile:str,outfile:str,gtf:str,writeMode:str,func:Callable[[str,set],list]=parse_gtf):
    ...
    bed.sort()
    ## remove the duplicates(chrom,start,end)
    bed = dropDuplicates(bed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="extract the gene region for TE and gene")
    ...
    if mode == "SNP":
        ...
        getBed(infile,outfile,Gtf,writeMode="w",func=parse_gtf)
        getBed(infile,outfile,TE_gtf,writeMode="a+",func=parase_gtfForTEcount)
    elif mode == "StringTie":
        if len(outfileList) == 2:
            getBed(infile,outfileList[0],Gtf,writeMode="w",func=parse_gtfForStringTie)
