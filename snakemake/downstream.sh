...
function mouse_SNP(){
    ...
    echo ${control_vcf[@]}
    echo ${experiment_vcf[@]}
    # python workflow/snakemake/scripts/SNP/plot.py \
    #     --mode vcf \
    #     --control ${control_vcf[@]} \
    ...
    python workflow/snakemake/scripts/SNP/alteredGene.py \
        --control ${control_vcf[@]} \
        --experiment ${experiment_vcf[@]} \
        --outdir ${outdir}/SNP/ \
        --species mouse 
    # Rscript workflow/snakemake/scripts/SNP/go-kegg.r \
    #     -m go kegg \
    #     -i ${outdir}/SNP/alteredGene.csv \
    #     -o ${outdir}/ \
    #     -s mouse
    ### annovar
...
export -f mouse_SNP
## GSE166216
group=workflow/snakemake/assets/GSE166216.csv
controlStr=mESC
experimentStr=2C
outdir=output/GSE166216
item=GSE166216
threshold=100
mouse_SNP ${group} ${controlStr} ${experimentStr} ${outdir} ${item} ${threshold}

## GSE183522
group=workflow/snakemake/assets/GSE183522.csv
controlStr=EPS
experimentStr=TPS
outdir=output/GSE183522
item=GSE183522
threshold=100
mouse_SNP ${group} ${controlStr} ${experimentStr} ${outdir} ${item} ${threshold}
## GSE185005
group=workflow/snakemake/assets/GSE185005.csv
controlStr=mESC
experimentStr=ciTotiSC
outdir=output/GSE185005
item=GSE185005
threshold=100
mouse_SNP ${group} ${controlStr} ${experimentStr} ${outdir} ${item} ${threshold}
...
export -f human_SNP
## GSE204801
group=workflow/snakemake/assets/GSE204801.csv
controlStr=prEpiSC
experimentStr=ci8CLC
outdir=output/GSE204801
item=GSE204801
threshold=150
human_SNP ${group} ${controlStr} ${experimentStr} ${outdir} ${item} ${threshold}
## GSE224794
group=workflow/snakemake/assets/GSE224794.csv
controlStr=hESC
experimentStr=hTBLC
outdir=output/GSE224794
item=GSE224794
threshold=80
human_SNP ${group} ${controlStr} ${experimentStr} ${outdir} ${item} ${threshold}

#########fusion gene
function mouse_fusion(){
    indir=$1
    python workflow/snakemake/scripts/fusion/volcanoImportant.py \
        --input ${indir}/2pass/mouse_StgTEOverlap.bed \
        --output ${indir}/DESeq2/plot/fusionGene.jpeg \
        --gtf ${indir}/2pass/mouse.gtf \
        --volcano ${indir}/DESeq2/TEcount_Gene.csv \
        --annFile /ChIP_seq_2/Data/index/Mus_musculus/GENCODE/GRCm39/geneIDAnnotation.csv \
        --TE "MERVL-int" "MT2_Mm" \
        --graphTitle "MERVL"
}
export -f mouse_fusion
GSE166216=output/GSE166216
GSE183522=output/GSE183522
GSE185005=output/GSE185005
# mouse_fusion ${GSE166216}
# mouse_fusion ${GSE183522}
# mouse_fusion  ${GSE185005}

function human_fusion(){
    indir=$1
    python workflow/snakemake/scripts/fusion/volcanoImportant.py \
        --input ${indir}/2pass/human_StgTEOverlap.bed \
        --output ${indir}/DESeq2/plot/fusionGene.jpeg \
        --gtf ${indir}/2pass/human.gtf \
        --volcano ${indir}/DESeq2/TEcount_Gene.csv \
        --annFile /ChIP_seq_2/Data/index/Homo_sapiens/GENCODE/GRCh38/geneIDAnnotation.csv \
        --TE "HERVH" \
        --graphTitle "HERVH"
}
export -f human_fusion
GSE204801=output/GSE204801
GSE224794=output/GSE224794
# human_fusion ${GSE204801}
# human_fusion ${GSE224794}
