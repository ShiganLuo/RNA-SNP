...
def combineTE(infiles: list[str],outfile: str,split:str):
    ...
    for infile in infiles:
        ...
        logger.info(f"sample name: {newColName}")
...
if __name__ == "__main__":
    ...
    if args.procedure == "TEcount":
        ...
        logger.info(f"TEcount.cntTable file path : {infiles}")
        combineTE(infiles,args.output,".TEcount")
        logger.info(f"write file into {args.output}")
    elif args.procedure == "TElocal":
        ### TElocal
        infiles = fileList(args.inputDir,".TElocal.cntTable")
        logger.info(f"TElocal.cntTable file path : {infiles}")
        ...
        logger.info(f"TEcountStringTie.cntTable file path : {infiles}")
        combineTE(infiles,args.output,".TEcountStringTie")
        logger.info(f"write file into {args.output}")
    else:
        logger.warning(f"Please check your procedure name {args.procedure},muste be TEcount or TElocal or TEcountStringTie")
        exit(1)
